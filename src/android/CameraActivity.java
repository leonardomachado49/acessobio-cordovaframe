...
public class CameraActivity extends Fragment {
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    ...
    int drawableFace = getResources().getIdentifier("face_frame_magenta", "drawable", appResourcesPackage);
    createCameraPreview(drawableFace);
    return view;
  }
  ...
  private void createCameraPreview(int drawableFace){
    if(mPreview == null) {
      setDefaultCameraId();
      ...
      //video view
      mPreview = new Preview(getActivity(), drawableFace);
  ...
  public static String toBase64(Bitmap bmp) {
    ...
    } else if (bmp.getWidth() == 720) {
      bmp.compress(Bitmap.CompressFormat.JPEG, 92, byteArrayOutputStream);
    } else {
      bmp.compress(Bitmap.CompressFormat.JPEG, 92, byteArrayOutputStream);
    }
  ...
  public static Bitmap rotateBitmap(Bitmap bitmap, int degree) {
    ...
    if (width < height) return bitmap; // Se já estiver na orientação portrait
  ...
  public void takePicture(final int width, final int height){
    Log.d(TAG, "CameraPreview takePicture width: " + width + ", height: " + height);
    ...
    if(mPreview != null) {
      ...
      Toast.makeText(this.getActivity(), "Processando, aguarde...", Toast.LENGTH_SHORT).show();
